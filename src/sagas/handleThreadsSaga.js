...
function* handleNewUser(client, identity, address) {
    ...
    let masterThreadIDBuffer = yield Textile.createMasterThreadDB(client, masterThreadName)
    let previewEntriesThreadIDBuffer = yield Textile.createMasterPreviewEntriesDB(client, masterThreadName)
    //  Set local sotage for future sessions and create user
    let masterThreadID = masterThreadIDBuffer.toString()
